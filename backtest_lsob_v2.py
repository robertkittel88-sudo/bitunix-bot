...
REPORT_FILE  = "backtest_lsob_results.json"
CSV_FILE     = "backtest_lsob_trades.csv"
...
def generate_lsob_signal(candles, current_idx, swing_highs, swing_lows, symbol="ETHUSDT"):
    """
    LSOB Signal-Logik:
    ...
    if signal == "BUY":
        # SL unter letztem Swing Low oder fester %
        prev_lows_list = [sl for sl in swing_lows if sl["idx"] < current_idx]
        if prev_lows_list:
            struct_sl = round(prev_lows_list[-1]["price"] * 0.999, 6)
            fixed_sl  = round(current_price * (1 - sl_pct), 6)
            sl = max(struct_sl, fixed_sl)  # Nehme den höheren (engeren) SL
        else:
            sl = round(current_price * (1 - sl_pct), 6)
        tp = round(current_price * (1 + tp_pct), 6)
    else:
        # SL über letztem Swing High oder fester %
        prev_highs_list = [sh for sh in swing_highs if sh["idx"] < current_idx]
        if prev_highs_list:
            struct_sl = round(prev_highs_list[-1]["price"] * 1.001, 6)
            fixed_sl  = round(current_price * (1 + sl_pct), 6)
            sl = min(struct_sl, fixed_sl)  # Nehme den niedrigeren (engeren) SL
        else:
            sl = round(current_price * (1 + sl_pct), 6)
        tp = round(current_price * (1 - tp_pct), 6)
...
def run_backtest_lsob(symbol, candles):
    ...
    for i in range(window, len(candles)):
        ...
        # LSOB Signal generieren
        sig = generate_lsob_signal(candles, i, swing_highs, swing_lows, symbol)
        if sig:
            open_trade = {
                "symbol":    symbol,
                "signal":    sig["signal"],
...
def print_results(stats):
    print()
    print(bold(green("╔══════════════════════════════════════════════════════════╗")))
    print(bold(green("║       LSOB BACKTEST v2 – ASSET-SPEZ. SL/TP              ║")))
    print(bold(green("╚══════════════════════════════════════════════════════════╝")))
...
def print_comparison(stats):
    prev = {
        "ETHUSDT":  {"win_rate": 29.0, "label": "Klassisch v1"},
        "HBARUSDT": {"win_rate": 36.5, "label": "Klassisch v1"},
    }
